...
| 页面 | 摘要 | 标签 |
|------|------|------|
| [[并发模型对比]] | Linux/JS/Rust/Cocos 四种并发模型的调度、隔离、通信三轴对比 | `#layer/sw-eng` `#type/comparison` |
| [[错误处理策略对比]] | 异常 vs 错误值——Linux/JS/Rust/Cocos 的哲学分歧与组合性对比 | `#layer/sw-eng` `#type/comparison` |
| [[内存管理对比]] | 虚拟内存(OS) → 所有权(Rust) → GC(JS) 的分层协作 | `#layer/sw-eng` `#type/comparison` |
| [[模块化对比]] | Rust crate → TS/ES 模块 → Linux LKM——从最严格到最特殊 | `#layer/sw-eng` `#type/comparison` |
| [[设计模式]] | 跨语言模式的形态差异——观察者在 JS/Cocos/Rust 中的不同实现 | `#layer/sw-eng` `#type/concept` |
| [[测试策略]] | 测试金字塔、Rust 语言内建 vs JS 生态拼装的哲学差异 | `#layer/sw-eng` `#type/concept` |

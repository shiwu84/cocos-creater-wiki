...
- [[Linux 文件系统]] — inode、层次结构、挂载、权限
- [[Linux 内核]] — 架构、特权级别、系统调用、内核模块
- [[Linux 网络栈]] — TCP/IP、路由、DNS、网络配置
- [[systemd 服务管理]] — 单元、目标、启动流程
- [[Linux 权限模型]] — rwx、SUID/SGID、用户管理
- [[Linux 启动流程]] — BIOS→GRUB→内核→init
- [[Linux 包管理]] — apt/yum/pacman、依赖解析
